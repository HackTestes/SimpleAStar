    ...
    while (OPEN.top().reference_key != local_goal)
    {
        ...
        for (long neighbor_index : my_neighbors_list)
        {
            ...
            if (!(barrier.find(neighbor_index) != barrier.end()))
            {
                ...
                {
                    my_map[neighbor_index].f = cost_so_far + heuristic_h(neighbor_index, local_goal);
                    ...
                    OPEN.push( PriorityQueueContainer<long>(my_map[neighbor_index].f, neighbor_index) ); // coloco de volta com o valor atualizado
                ...
                if (!my_map[neighbor_index].in_priority_queue && !my_map[neighbor_index].visited)
                {
                    ...
                    OPEN.push( PriorityQueueContainer<long>(my_map[neighbor_index].f, neighbor_index) );
